...
for fl in `find MR018001_MR019000 -name "MR*.txt"|xargs -P10 -n20 grep -l "Consultation Details"`
do
dir=`dirname $fl`
file=`basename $fl`
...
find MR018001_MR019000 -name "fwdiag*"|xargs awk '{print FILENAME, $0}' > fwdiag_all_18_19000.txt
cat fwdiag_all_18_19000.txt |tr -s "\t" " " > fwdiag02_all_18_19000.txt

# Find files with @ symbol
#/cygdrive/d/Hospital_data/04_2017_DOWNLOAD/pat_txts_diagnosis
find MR018001_MR019000 -name "diag*"|xargs grep "@"|cat -n
...
dta <- fread("D:\\Hospital_data\\04_2017_DOWNLOAD\\pat_txts_diagnosis\\fwdiag02_all_18_19000.txt", 
             header=F)
...
chk470 <- chk47 [, `:=` (awkst1 = "gawk 'BEGIN {FIELDWIDTHS=\"",
                         ...
                         awkst3 = "' ",
                         awkst4 = paste( "diag_", substr(TMP2, 8, 70), sep=""),
                         awkst5 = paste( "aligndiag_", substr(TMP2, 8, 50), sep="" )),]

chk471 <- chk470 [, code := paste(awkst1, text, awkst2, awkst3, TMP1, "/", awkst4, "> ", TMP1, "/", awkst5, sep = ""),]

write.table(chk471$code, 
            "D:\\Hospital_data\\04_2017_DOWNLOAD\\pat_txts_diagnosis\\awk_diag_18_19000.sh", 
            row.names=FALSE, 
            col.names=FALSE,
            quote= FALSE)
...
# dos2unix awk_diag_18_19000.sh
# chmod 777 awk_diag_18_19000.sh
# bash awk_diag_18_19000.sh

# Run the AWK code to get all the aligned files into 1 file
find MR018001_MR019000 -name "aligndiag*"|xargs awk '{print FILENAME "@" $0}' >all_aligndiag_18_19000.txt

all_dose1819 <- fread("D:\\Hospital_data\\04_2017_DOWNLOAD\\pat_txts_diagnosis\\all_aligndiag_18_19000.txt",
                  header=F, 
                  sep="@",
                  fill=TRUE)
...
all_dose00 <-rbind(all_dose, all_dose1819)
all_dose0 <- all_dose00 [V2 != "Diag__Type"]
